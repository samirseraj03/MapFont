    ...
    try {
      ...
      this.map.on('load', () => {
        ...
        this.map.on('moveend', () => {
          ...
          console.log('Nueva ubicación después del zoom:', center, 'Nivel de zoom:', zoom);
    
          // Aquí puedes hacer lo que necesites con la nueva ubicación y el nivel de zoom
          // Por ejemplo, puedes realizar una solicitud para obtener datos específicos basados en la nueva ubicación
          const features = this.map.querySourceFeatures('watersources', {
            filter: ['all', 
            ['>=', zoom , 13], // Filtro para características que deben mostrarse solo a cierto nivel de zoom
            ['within', [center.lng, center.lat ]] // Área visible del mapa
          ] // Filtro para características que deben mostrarse solo a cierto nivel de zoom
          });
        ...
        this.map.on('click', 'clusters', (e : any) => {
          ...
          this.map.getSource('watersources').getClusterExpansionZoom(
              clusterId,
              (err: any, zoom : any) => {
                  if (err) return;
